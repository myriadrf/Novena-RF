...
void SelectionContainer::add(const string& str)
{
	sItems.push_back(str);
};
void SelectionContainer::add(const char* ch)
{
	string stmp = ch;
	sItems.push_back(stmp);
}
void SelectionContainer::add(const int integer)
{
	char ctmp[32];
	memset (ctmp,'\0',32);
...
int SelectionContainer::indexOf(const string &str)
{
