...
BEGIN_EVENT_TABLE(pnlSpectrum,wxPanel)
	...
	//*)
	//EVT_COMMAND  (GRAPH_UPDATE_MESSAGE_ID, wxEVT_COMMAND_TEXT_UPDATED, pnlSpectrum::onUpdateGraphs)
END_EVENT_TABLE()
...
pnlSpectrum::pnlSpectrum(wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size, int style, wxString str)
{
    ...
    t_graphUpdate = new GraphUpdateThread(this);
}
...
void pnlSpectrum::BuildContent(wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size)
{
	...
	FlexGridSizer8->Add(chkAverage, 0, wxTOP|wxLEFT|wxALIGN_LEFT|wxALIGN_CENTER_VERTICAL, 5);
...
void pnlSpectrum::initializeGraphs()
{
    ...
    if(ogl_FFTline)
    {
        ...
        ogl_FFTline->settings.marginLeft = 70;
...
/**
	@brief Updates displayed info and redraw graphs with new values.
*/
void pnlSpectrum::UpdateGraphs()
{
...
/**
	@brief Generates values inside GUI comboboxes and selects default values
*/
...
{
    LMLL_TxLPFSetLpfBw(cmbTxFilter->GetSelection()+4); // +4 because this combobox has limited items
}

void pnlSpectrum::OncmbRxFilterSelected(wxCommandEvent& event)
{
    LMLL_RxLPFSetLpfBw(cmbRxFilter->GetSelection()+4); // +4 because this combobox has limited items
}
...
void pnlSpectrum::OnSpinButton1Change(wxSpinEvent& event)
{
}
...
void pnlSpectrum::OnEraseBackground(wxEraseEvent& event)
{
}
